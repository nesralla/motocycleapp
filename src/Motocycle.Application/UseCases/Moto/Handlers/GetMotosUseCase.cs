...
{
    public class GetMotosUseCase : UseCaseBaseRequestToDomain<GetMotosRequest, Motocy, List<MotoResponse>>
    {
        private readonly IMotocyService _motocyService;
        public GetMotosUseCase(
        ...
        public override async Task<List<MotoResponse>> HandleSafeMode(GetMotosRequest request, CancellationToken cancellationToken)
        {
            var motocyList = await _motocyService.GetAllQueryAsNoTracking.ToListAsync();
            var response = Mapper.Map<List<MotoResponse>>(motocyList);
            return response;
        }
